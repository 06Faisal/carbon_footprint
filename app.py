...
def get_user_stats(user):
    ...
    c.execute("SELECT COUNT(DISTINCT date) FROM trips WHERE user_name=%s", (user,))
    active_days = c.fetchone()[0]

    c.execute("SELECT COUNT(*) FROM trips WHERE user_name=%s", (user,))
    total_trips = c.fetchone()[0]

    c.execute("SELECT COUNT(*) FROM trips WHERE user_name=%s AND vehicle IN ('Bus','Train')", (user,))
    public_trips = c.fetchone()[0]

    c.execute("SELECT COUNT(*) FROM trips WHERE user_name=%s AND vehicle='Bike'", (user,))
    bike_trips = c.fetchone()[0]

    c.execute("SELECT COUNT(DISTINCT month) FROM electricity WHERE user_name=%s", (user,))
...
def daily_travel():
    ...
    if request.method == "POST":
        ...
        c.execute("""
            INSERT INTO trips (user_name, mode, vehicle, distance, date)
...
def end_trip():
    ...
    c.execute("""
        INSERT INTO trips (
            user_name, mode, vehicle, distance, date,
...
def delete_trip(trip_id):
    db = get_db()
    c = db.cursor()
    c.execute("DELETE FROM trips WHERE id=%s AND user_name=%s", (trip_id, session["user"]))
    db.commit()
    db.close()
...
def emissions():
    ...
    c.execute("SELECT vehicle, distance, mode FROM trips WHERE user_name=%s AND date=%s",
              (user, today))
    rows = c.fetchall()
    ...
    c.execute("SELECT SUM(co2) FROM electricity WHERE user_name=%s AND month=%s",
              (user, current_month))
...
def electricity():
    ...
    if request.method == "POST":
        ...
        db = get_db()
        c = db.cursor()
        c.execute("""INSERT INTO electricity (user_name, month, units, co2, bill_file, uploaded_at)
                     VALUES (%s, %s, %s, %s, %s, %s)""",
...
def suggestions():
    ...
    db = get_db()
    c = db.cursor()
    c.execute("SELECT vehicle, distance FROM trips WHERE user_name=%s AND date=%s", (user, today))
    rows = c.fetchall()

    travel_emission = round(sum(d * EMISSION_FACTORS.get(v, 0) for v, d in rows), 2)

    c.execute("SELECT SUM(co2) FROM electricity WHERE user_name=%s", (user,))
    electricity_emission = round(c.fetchone()[0] or 0, 2)
    db.close()
...
def leaderboard():
    ...
    for username in users:
        ...
        c.execute("SELECT vehicle, distance FROM trips WHERE user_name=%s", (username,))
        rows = c.fetchall()
        ...
        c.execute("SELECT SUM(co2) FROM electricity WHERE user_name=%s", (username,))
        electricity_emission = round(c.fetchone()[0] or 0, 2)
